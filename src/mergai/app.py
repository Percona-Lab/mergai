...
from .solution_types import CONFLICT_RESOLUTION
from .utils import git_utils, util
from .utils.branch_name_builder import BranchNameBuilder
from .utils.formatters import _format_file_section, _render_solution_body
...
class AppContext:
    ...
    def commit_ci_fix_solution(self, solution_idx: int) -> None:
        ...
        Thin delegator to :func:`mergai.ci.commit.commit_ci_fix_solution`,
        which holds the CI-fix commit logic alongside the rest of the
        ``mergai.ci`` package.
        """
        from .ci.commit import commit_ci_fix_solution

        commit_ci_fix_solution(self, solution_idx)
    ...
    def commit_solution(self):
        ...
        # Mark conflict markers as unresolved and stage those files. Done
        # before rendering the body, which only builds the message string.
        ...
        message += _render_solution_body(
            solution["response"].get("summary", ""),
            solution["response"].get("resolved", {}),
            unresolved_files,
            modified_files,
            unresolved_label="Unresolved (contains conflict markers)",
        )
    ...
    def _build_squash_commit_message(
        ...
        # weren't already shown under Conflicts).
