...
def _safe_extract(zf: zipfile.ZipFile, target_dir: Path) -> None:
    """Extract all members into ``target_dir``, refusing any that escape it.

    Guards against zip-slip / path traversal (entries like ``../../x`` or
    absolute paths): artifacts come from workflow output, which on
    PR-triggered runs can include untrusted input. Raises ``ValueError`` on
    the first unsafe member, before extracting anything.
    """
    ...
    for name in zf.namelist():
        ...
        if dest != base and not dest.is_relative_to(base):
            raise ValueError(
                f"Refusing to extract unsafe path '{name}' (escapes {base})"
            )
...
def _resolve_artifact_zip_url(run: WorkflowRun, artifact: object) -> str:
    ...
    # Header key casing isn't guaranteed ("location" vs "Location"); look up
    # case-insensitively and fail clearly if the redirect target is missing.
    location: str | None = next(
        (v for k, v in headers.items() if k.lower() == "location"), None
    )
    if location is None:
        raise urllib.error.URLError(
            f"No redirect Location header for artifact zip URL {api_url}"
        )
    return location
