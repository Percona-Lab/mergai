...
class WorkflowResolutionConfig:
    """Resolution strategy configuration for a CI workflow.

    Defines how to resolve failures for a specific workflow.

    Attributes:
        type: Resolution strategy type. Options:
            - 'command': Run a shell command to fix the issue
            - 'agent': Use AI agent to analyze and fix the issue
        command: Shell command to execute (only used when type='command').
    ...
    Example YAML config:
        ...
        # Or for agent-based resolution:
        resolution:
          type: agent
    """

    type: str = "agent"  # "command" | "agent"
    command: str | None = None

    @classmethod
    def from_dict(cls, data: dict) -> "WorkflowResolutionConfig":
        """Create a WorkflowResolutionConfig from a dictionary.

        Args:
            data: Dictionary with configuration values.

        Returns:
            WorkflowResolutionConfig instance with values from data.

        Raises:
            ValueError: If type is not 'command' or 'agent'.
        """
        resolution_type = data.get("type", cls.type)
        if resolution_type not in ("command", "agent"):
            raise ValueError(
                f"Invalid resolution type: '{resolution_type}'. "
                "Valid values are 'command' or 'agent'."
            )

        command = data.get("command")
        if resolution_type == "command" and not command:
            raise ValueError("Resolution type 'command' requires a 'command' field.")
...
class WorkflowConfig:
    """Configuration for a single CI workflow.

    Defines how mergai should handle failures from a specific workflow.

    Attributes:
        name: Name of the workflow (must match GitHub workflow name).
        enabled: Whether to handle failures for this workflow.
        max_attempts: Maximum number of fix attempts before giving up.
        resolution: Resolution strategy configuration.
    ...
    Example YAML config:
        - name: format
          ...
          resolution:
            type: command
            command: "bazel run format --include_autogenerated_targets=True"
    """

    name: str
    enabled: bool = True
    max_attempts: int = 3
    resolution: WorkflowResolutionConfig = field(
        default_factory=WorkflowResolutionConfig
    )
    ...
    def from_dict(cls, data: dict) -> "WorkflowConfig":
        ...
        Returns:
            WorkflowConfig instance with values from data.
        ...
        name = data.get("name")
        if not name:
            raise ValueError("Workflow config requires a 'name' field.")

        resolution_data = data.get("resolution", {})
        resolution = (
            WorkflowResolutionConfig.from_dict(resolution_data)
            if resolution_data
            else WorkflowResolutionConfig()
        )
...
@dataclass
class ChecksConfig:
    """Configuration for CI check handling.

    Controls how mergai handles CI workflow failures, including which
    workflows to monitor and how to resolve failures.
    ...
    def get_enabled_workflows(self) -> list[WorkflowConfig]:
        """Get all enabled workflow configs.

        Returns:
            List of enabled WorkflowConfig instances.
        """
        return [w for w in self.workflows if w.enabled]

    def get_workflow_names(self) -> list[str]:
        """Get names of all configured workflows.
    ...
    @classmethod
    def from_dict(cls, data: dict) -> "ChecksConfig":
        """Create a ChecksConfig from a dictionary.

        Args:
            data: Dictionary with configuration values.

        Returns:
            ChecksConfig instance with values from data.
        """
        ...
        for workflow_data in workflows_data:
            try:
                ...
                log.warning(f"Skipping invalid workflow config: {e}")
...
class MergaiConfig:
    ...
    Attributes:
        ...
        checks: Configuration for CI check handling.
    ...
    def from_dict(cls, data: dict) -> "MergaiConfig":
        ...
        # Parse checks section if present
        checks_data = data.get("checks", {})
        checks_config = (
            ChecksConfig.from_dict(checks_data) if checks_data else ChecksConfig()
        )
