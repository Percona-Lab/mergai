    ...
    *,
    ...
    Args:
        ...
        check_failure_kind: When False, a ``failure`` run is reported
            ...
            run with a resolved PR inspects its jobs (one ``run.jobs()`` call)
            to detect a fail-fast-masked failure, since its cheap default is to
            skip rather than act (a ``cancelled`` run with no PR is skipped
            outright, without the jobs call - see the ``pr_number is not None``
            guard below). That call is bounded to head-current cancelled runs,
            but ``ci list`` does pay it even with ``check_failure_kind=False``.
